 ...
 */
public class AddData_FaceDetection extends javax.swing.JFrame {
    ...
    CascadeClassifier faceDetector = new CascadeClassifier(AddData_FaceDetection.class.getResource("haarcascade_frontalface_alt.xml").getPath().substring(1));
    ...
    class DaemonThread implements Runnable {
        ...
        private void saveBufferedImageAndAssocatedData(BufferedImage image, person.data.PersonData person) throws IOException {
            ...
            int userID = 0;
            int count = new File("C:\\Imgs").listFiles(pngFilter).length;
            if (count == 0) {
                userID = 1;
            } else {
                userID = new File("C:\\Imgs").listFiles(pngFilter).length + 1;
            }

            try {
                ImageIO.write(image, "PNG", new File("C:\\Imgs\\" + userID + ".png"));
            } catch (Exception ex) {
                 ex.printStackTrace();
            }

            try {
                PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("C:\\Imgs\\Data.txt", true)));
                out.println(userID + "-" + person.getName() + "-" + person.getAge() + "-" + person.getAddress() + "-" + person.getPhone());
                out.close();
            } catch (IOException e) {
                  e.printStackTrace();
            }
                        ...
                        try {
                            ...
                            if (g.drawImage(buff, 0, 0, getWidth(), getHeight() - 150, 0, 0, buff.getWidth(), buff.getHeight(), null)) {
                                if (runnable == false) {
                                    ...
                                    person.setAddress(jTextField4.getText());
                                    person.setAge(jTextField2.getText());
                                    person.setName(jTextField1.getText());
                                    person.setPhone(jTextField3.getText());
                                    if (rect_Crop != null) {
                                        Mat image_roi = new Mat(frame, rect_Crop);
                                        saveBufferedImageAndAssocatedData(ImpageCropping.MatToBufferedImage(image_roi), person);
     ...
     */
    public AddData_FaceDetection() {
        initComponents();
        System.out.println(AddData_FaceDetection.class.getResource("haarcascade_frontalface_alt.xml").getPath().substring(1));
    }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(AddData_FaceDetection.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
